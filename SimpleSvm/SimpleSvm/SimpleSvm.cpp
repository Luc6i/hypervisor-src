    ...
    while (StopCounterThread == FALSE)
    {
        if (TargetProcessId)
        {
            ...
            if (NewKuserSharedData)
            {
                ...
                *(UINT64*)(NewKusdAddress + 0x000) = 0x0FA0000000000000;
                ...
                *(UINT8*)(NewKusdAddress + 0x297) = 0x0;    // Disable RDRAND usage
                ...
                // XSAVE related stuff
                *(UINT8*)(NewKusdAddress + 0x285) = 0x0;    // Disable XSAVE usage
                *(UINT8*)(NewKusdAddress + 0x29B) = 0x0;    // Disable AVX usage
                *(UINT8*)(NewKusdAddress + 0x29C) = 0x0;    // Disable AVX2 usage
    ...
    if (VpData->GuestVmcb.StateSaveArea.Cpl == 0x3)
    {
        ...
        if (VpData->GuestVmcb.StateSaveArea.Cr3 == TargetCR3)
        {
            if (leaf == 0x1)
            {
                ...
                //GuestContext->VpRegs->Rcx = 0x76F8320B; // Disabled XSAVE via bcdedit
                //GuestContext->VpRegs->Rcx = 0x7AF8220B; // Disabled FMA3 and XSAVE
                GuestContext->VpRegs->Rcx = 0x72F8220B; // Disabled FMA3 and XSAVE and OSXSAVE
                //GuestContext->VpRegs->Rcx = 0x7EF8220B; // Disabled FMA3
            ...
            {
                //SvDebugPrint("Setting syscall handler at %p", reinterpret_cast<void*>(VpData->GuestVmcb.StateSaveArea.Rip));
